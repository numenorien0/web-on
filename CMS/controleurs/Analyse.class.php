	...
	public function __construct()
	{
		...
		if(isset($_GET['date']) AND $_GET['date'] != null)
		{
			switch($_GET['date'])
			{
				...
				break;
				case "30": 
					$this->_time = "WHERE date > '".(time()-2629800)."' ";
...
*/
		...
		$sql = "SELECT REPLACE(page,'www.', ''), count(*) FROM tracking_follow $this->_time GROUP BY REPLACE(page,'www.', '') ORDER BY COUNT(*) DESC LIMIT 0, 40";
		$reponse = $this->_db->query($sql);
		
		while($donnees = $reponse->fetch())
		{
			$page = $donnees['REPLACE(page,\'www.\', \'\')'];
			$pagePosition = strripos($page, "/");
			$page = substr($page, $pagePosition);
			if($page == "/")
			{
				$page = "Accueil";
			}
			echo "<div class='pageLesPlusVues' data-value='".$donnees['count(*)']."' data-label='".$page."'></div>";
			#echo $donnees['count(*)']." ".$donnees['REPLACE(page,\'www.\', \'\')']."<br/>";
		}
		echo "<div class='pageContainer col-lg-10 col-lg-offset-1 col-sm-12 cadre'>";
		echo "<h3>Pages les plus vues</h3>";
		echo '<canvas id="pagesVues" width="" height="300" style="display: block; margin: auto; margin-bottom: 0px"></canvas>';
		echo "</div>";
		...
		echo "<div id='dialogComportement'></div>";
		while($i <= $step)
		{
			...
			if($i == 0)
			{
				echo "<h6>Arrivée <div class='pagePerte'> $totalConnexion visiteurs </div></h6>";
			}
			else
			{
				echo "<h6>".$interaction. " <div class='pagePerte'> $totalConnexion visiteurs </div></h6>";
				#echo "";
			echo "<div data-height='".number_format($calculPerte, 2, ',',' ')."' class='pageInteraction pageInteractionDeco'><div class='decoTitle'>Déconnexion</div><div class='deconnexionPercent'>".number_format($calculPerte, 2, ',',' ')."%</div><div data-value='".number_format($calculPerte, 2, ',',' ')."' class='remplissageRed'></div><div class='whatIsTheNextStep'><span class='ensuite'></span></div></div>";
			}
			//<span class='pourcentagePerte'>(".number_format($calculPerte, 2, ',',' ')." % &darr;)</span>
			...
			foreach($majoritePage as $key => $page)
			{
				...
				if($calcul > 10)
				{
					// <span class='pourcentage'>(".number_format($calcul, 2, ',',' ')."%)
					echo "<div data-value='".number_format($calcul, 2, ',',' ')."' data-height='".number_format($calcul, 2, '.',' ')."' class='pageInteraction' title='".$page."'><div data-value='".number_format($calcul, 2, ',',' ')."' class='remplissage'></div><div class='pageTitle pageInteractive'>".$page."</div><div class='whatIsTheNextStep'> <span class='ensuite'>".number_format($calcul, 2, '.',' ')." %</span><br/><br/>";
					$trouve = false;
					arsort($tableauTrie);
					echo "Ensuite ?<br/>";
			...
			if($autrePage != 0)
			{
				echo "<div data-height='".number_format($autrePage, 2, '.',' ')."' class='pageInteraction'><div class='pageTitle'>Autres</div><div data-value='".number_format($autrePage, 2, ',',' ')."' class='remplissage'></div><div class='whatIsTheNextStep'><span class='ensuite'>".number_format($autrePage, 2, ',',' ')."%</span></div></div>";				
			}
	...
	public function readStat()
	{
		...
		if(!isset($_GET['date']))
		{
			...
			{
				$mois = $boucle-1;
// 				$moisActuel = strtotime("- $boucle month", strtotime("02/28/".date('Y')));
				$base = strtotime("- $boucle month", strtotime(date('m')."/28/".date('Y')));
// 				echo $base;
				$baseMoinsUn = strtotime("- $mois month", strtotime(date('m')."/28/".date('Y')));
	
				$base = date("m/t/Y", $base);
				$base = strtotime($base);
				$baseMoinsUn = date("m/t/Y", $baseMoinsUn);
				$baseMoinsUn = strtotime($baseMoinsUn);
				$sqlTest = "SELECT * FROM tracking WHERE date >= $base AND date <= $baseMoinsUn";
				$reponseTest = $this->_db->query($sqlTest);
				setlocale(LC_TIME, "fr_FR");
				$dateToDisplay = utf8_encode(strftime("%B %Y", $baseMoinsUn));
// 				date("F/Y", $baseMoinsUn)
				echo "<div class='traficSurDixJours' data-value='".$reponseTest->rowCount()."' data-date='".ucfirst($dateToDisplay)."'></div>";
				$boucle++;
		...
		else
		{
			if($calcul != 1)
			{
				while($boucle <= $calcul)
				{
					$date = date("m/d/Y 00:00", strtotime('+1 day'));
					$date = strtotime($date);
					#$heure = date("m/d/Y ".$heure.":00");
					#$heure = strtotime($heure);
					
					$base = $date - ($boucle*86400);
					$baseMoinsUn = $date - (($boucle-1)*86400);
		
					$sqlTest = "SELECT * FROM tracking WHERE date >= $base AND date <= $baseMoinsUn";
					$reponseTest = $this->_db->query($sqlTest);
					echo "<div class='traficSurDixJours' data-value='".$reponseTest->rowCount()."' data-date='".date("d/m/Y", $base)."'></div>";
					$boucle++;
					$titre = "$calcul derniers jours";
				}
			}
			else
			{
				...
				while($boucle <= 24)
				{
					$heure = date("m/d/Y H:00", strtotime('+1 hour'));
					$heure = strtotime($heure);
					#$heurePlusUn = strtotime("+1 jo$heure+1;
					//$heure = strtotime("m/d/Y : $heure");
					
					//$heure = strtotime("+1 hour", $heure);
					
					
					//echo $heure;
					$base = $heure - ($boucle*3600);
					$baseMoinsUn = $heure - (($boucle-1)*3600);
		
					$sqlTest = "SELECT * FROM tracking WHERE date >= $base AND date <= $baseMoinsUn";
					$reponseTest = $this->_db->query($sqlTest);
					echo "<div class='traficSurDixJours' data-value='".$reponseTest->rowCount()."' data-date='".date("H:00", $base)."'></div>";
					$boucle++;
					$titre = "24 dernières heures";
					
				}			
			}
		}
		
		//print_r($donneesPourGraphique);
		...
		{
			echo "<div class='traficContainer col-sm-12 col-lg-10 col-lg-offset-1 cadre'>";
			echo "<h3>Le trafic sur les $titre </h3>";
			echo '<canvas id="traficGraph" width="" height="200" style="display: block; margin: auto; margin-bottom: 30px"></canvas>';
			echo "</div>";
			...
			echo "<div class='ligneValeur'><span class='green'>".number_format($reponse->rowCount(),0, ',', '.')."</span> visites ".$progression."</div>";
...
// 			echo $moyenneTempsDeVisiteParSession;
			...
			echo "<div class='ligneValeur'><span class='green'>".number_format($reponse10->rowCount(),0, ',', '.')."</span> personnes différentes</div>";
			...
			$total = $visiteTotale;
			echo '<canvas id="paysGraph" width="220" height="220" style="float: right; width: 220px; display: block; margin: auto; margin-bottom: 30px"></canvas>';
			while($donnees10 = $reponse10->fetch())
			{
				...
				echo "<div class='originGraphData' data-pays='".$pays."' data-pourcentage='".str_replace(',','.',$pourcentage)."'></div>";
			}
			echo "</div>";
			echo "<div class='cadre col-sm-12'>";
			echo "<h3>Langues</h3>";
			...
			echo '<canvas id="navGraph" width="200" height="200" style="width: 200px; float: right; display: block; margin: auto; margin-bottom: 30px"></canvas>';
			...
			while($donnees11 = $reponse11->fetch())
			{
				...
				echo "<div class='navigateurGraph' data-value='".str_replace(",",".", $pourcentageBrowser)."' data-label='".ucfirst($navigateur)."'></div>";
			}
			echo "</div>";	
			echo "<div class='cadre col-sm-12'>";
				...
				if($donnees11['referrer'] != null)
				{
					...
					else
					{
						if((strstr($url1['host'], "google") AND !strstr($url1['host'], "plus.url.google"))  OR strstr($url1['host'], "yahoo") OR strstr($url1['host'], "bing") OR strstr($url1['host'], "duckduckgo"))
						{
							...
							if(strstr($url1['host'], "google"))
							{
								$site = "Google";
								$condition = true;
							}
							if(strstr($url1['host'], "tpc.googlesyndication"))
							{
								$site = "Google Adsense";
								$condition = true;
							}						
							if(strstr($url1['path'], "aclk") OR strstr($url1['path'], "gclid"))
							{
								$site = "Google Adwords";
								$condition = true;
							}
							if(strstr($url1['host'], "images.google"))
							{
								$site = "Google images";
								$condition = true;
							}
							if(strstr($url1['host'], "googleads.g.doubleclick"))
							{
								$site = "Google Adsense";
								$condition = true;
							}
							...
							if(strstr($url1['host'], "search.yahoo"))
							{
								$site = "Yahoo";
								$condition = true;
							}								
							
							if(strstr($url1['host'], "bing"))
							{
								$site = "Bing";
								$condition = true;
							}	
						...
						{
							if(strstr($url1['host'], "facebook") OR strstr($url1['host'], "twitter") OR strstr($url1['host'], "t.co") OR strstr($url1['host'], "linkedin") OR strstr($url1['host'], "instagram") OR strstr($url1['host'], "youtube") OR strstr($url1['host'], "vimeo") OR strstr($url1['host'], "pinterest") OR strstr($url1['host'], "tumblr") OR strstr($url1['host'], "plus.url.google"))
							{	
								$positionExtension = strrpos($site, ".");
								$site = substr($site, 0, $positionExtension);
								...
								if(strstr($url1['host'], "t.co"))
								{
									$site = "twitter";
								}
								if(strstr($url1['host'], "lfacebook") OR strstr($site, "lfacebook"))
								{
									$site = "facebook";
								}
								
								if(strstr($url1['host'], "l.facebook"))
								{
									$site = "facebook";
								}
								$array_social[] = $site;
			...
			foreach($array_other as $other)
			{
				...
				$tableauDeTri[$other] = $moyenneOther; 
				//echo "<div class='ligneValeurDetails'>- ".$other." (<span class='blue'>".$moyenneOther."%</span>)</div>";
			}
			
			arsort($tableauDeTri);
			//print_r($tableauDeTri);
			
			foreach($tableauDeTri as $key => $tab )
			{
				echo "<div class='ligneValeurDetails'>- ".$key." (<span class='blue'>".$tab."%</span>)</div>";
			}
