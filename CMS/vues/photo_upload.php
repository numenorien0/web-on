...
else
{
    if($_GET['demand'] == "list")
    {
        ...
        {
            echo "<div class='photo col-sm-3 col-lg-2'><div class='col-sm-12' data-id='".$donnees['ID']."' style='background: url(content/thumbnail/".$donnees['fichier']."); background-size: cover; background-position: center'></div><div class='blackHover'><img data-img='content/album/".$donnees['fichier']."' class='fullscreen' src='images/fullscreen.png'><img data-id='".$donnees['ID']."' class='editIcon' src='images/pencil.png'><img data-id='".$donnees['ID']."' class='deleteIcon' src='images/delete.png'></div><div class='descriptionForm cadre'><h3>Description</h3><input type='hidden' value='".$donnees['ID']."' class='hiddenIDDescription'/><label class='col-sm-12' for='description'>Légende</label><input type='text' class='descriptionValue col-sm-12' value=\"".$donnees['description']."\" /><input style='margin-top: 20px' type='button' class='submitDescription' value='Enregistrer'/></div></div>";
        }
    }
